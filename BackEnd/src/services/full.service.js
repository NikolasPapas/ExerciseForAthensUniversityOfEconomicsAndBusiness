...
export class FullService {
	async getAll() {
		const owners = await ownerRepository.getOwner({});
		const vehicles = await vehicleRepository.getVehicle({});
		const insurances = await insuranceRepository.getInsurance({});

		const ownerVehicleModels = [];
		for (const owner of owners) {
			const filteredVehicles = vehicles.filter(
				(v) => v.ownerTaxId === owner.ownerTaxId
			);
			const vehicleInsuranceModels = [];
			for (const vehicle of filteredVehicles) {
				const filteredInsurance = insurances.filter(
					(i) =>
						i.plateNumber === vehicle.plateNumber &&
						i.ownerTaxId === vehicle.ownerTaxId
				);
